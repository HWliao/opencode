...
- **Do NOT run this module from inside a child-agent prompt template.** A child agent that follows Step 3 would spawn another child agent, which breaks the orchestration rules in `references/orchestration-patterns.md`.
...
The prompt templates in `agents/` can be used for isolated review passes when dispatched by the main session.

**The adversarial prompt above takes precedence over the child-agent template's default response shape.** Templates like `code-reviewer` are written to produce balanced verdicts with both strengths and weaknesses; doubt-driven needs issues-only output. Paste the adversarial prompt verbatim into the invocation so it overrides the template's default. If a template's response shape can't be overridden cleanly, fall back to a generic subagent with the adversarial prompt.
...
- **Repo orchestration rules** (`references/orchestration-patterns.md`): this module orchestrates from the main session. A child agent calling another child agent is forbidden — see Loading Constraints above.
